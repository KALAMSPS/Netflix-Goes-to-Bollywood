...
namespace secondWeek_Lu
{
    public class NewMCQDialogBox : MonoBehaviour
    {
        ...
        // public ThirdPersonController playerController;c
            ...
            while (currentDialogueIndex < dialogues.Length)
            {
                ...
                if (isPlayer1Turn && dialogue.requiresToggleMCQ)
                {
                    ...
                    if (dialogue.toggleMCQBox != null)
                    {
                        ...
                        if (toggleQuestionDatabase.TryGetValue(dialogue.toggleMCQID, out ToggleQuestionData toggleData))
                        {
                ...
                // Player 2 always advances to the next dialogue.
                // Player 1 advances too, unless Player 2's turn isn't being skipped.
                if (!isPlayer1Turn || dialogue.skipPlayer2)
                {
                    currentDialogueIndex++;
                    isPlayer1Turn = true;
        ...
        void ShowSingleMCQ(Dialogue dialogue)
        {
            ...
            if (mcqDatabase.TryGetValue(dialogue.mcqID, out MCQData data))
            {
        ...
        IEnumerator ShowMultipleMCQs(Dialogue dialogue)
        {
            ...
            for (int i = 0; i < dialogue.mcqIDs.Count; i++)
            {
                ...
                if (!mcqDatabase.TryGetValue(mcqID, out MCQData data))
                {
